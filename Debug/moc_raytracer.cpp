/****************************************************************************
** Meta object code from reading C++ file 'raytracer.h'
**
** Created: Sun 10. Mar 20:20:47 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../raytracer.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'raytracer.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_RayTracer[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: signature, parameters, type, tag, flags
      17,   11,   10,   10, 0x05,
      36,   10,   10,   10, 0x05,

 // slots: signature, parameters, type, tag, flags
      61,   57,   10,   10, 0x0a,
      84,   57,   10,   10, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_RayTracer[] = {
    "RayTracer\0\0value\0sigBucketDone(int)\0"
    "sigThreadsFinished()\0nId\0"
    "slotThreadStarted(int)\0slotThreadEnded(int)\0"
};

void RayTracer::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        RayTracer *_t = static_cast<RayTracer *>(_o);
        switch (_id) {
        case 0: _t->sigBucketDone((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->sigThreadsFinished(); break;
        case 2: _t->slotThreadStarted((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->slotThreadEnded((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData RayTracer::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject RayTracer::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_RayTracer,
      qt_meta_data_RayTracer, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &RayTracer::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *RayTracer::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *RayTracer::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_RayTracer))
        return static_cast<void*>(const_cast< RayTracer*>(this));
    return QObject::qt_metacast(_clname);
}

int RayTracer::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void RayTracer::sigBucketDone(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void RayTracer::sigThreadsFinished()
{
    QMetaObject::activate(this, &staticMetaObject, 1, 0);
}
QT_END_MOC_NAMESPACE
